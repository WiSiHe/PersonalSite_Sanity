...
var handleNavigate = urlStateStore.navigate; // eslint-disable-next-line @typescript-eslint/no-empty-interface
...
class DefaultLayoutRoot extends _react.default.PureComponent {
  ...
  UNSAFE_componentWillMount() {
    ...
    this.urlStateSubscription = urlStateStore.state.subscribe({
      next: event => {
        var urlState = this.state.urlState;
        var isNotFound = this.state.isNotFound;
        var intent = this.state.intent;

        if (!(0, _isEqual2.default)(this.state.urlState, event.state)) {
          urlState = event.state;
        }

        if (!(0, _isEqual2.default)(this.state.isNotFound, event.isNotFound)) {
          isNotFound = event.isNotFound;
        }

        if (!(0, _isEqual2.default)(this.state.intent, event.intent)) {
          intent = event.intent;
        }

        var urlStateEqual = this.state.urlState === urlState;
        var isNotFoundEqual = this.state.isNotFound === isNotFound;
        var intentEqual = this.state.intent === intent;
  ...
  render() {
    ...
    var content = isNotFound ? /*#__PURE__*/_react.default.createElement(_main.NotFound, null, intent && /*#__PURE__*/_react.default.createElement("div", null, "No tool can handle the intent: ", /*#__PURE__*/_react.default.createElement("strong", null, intent.name), " with parameters", ' ', /*#__PURE__*/_react.default.createElement("pre", null, JSON.stringify(intent.params)))) : /*#__PURE__*/_react.default.createElement(_defaultLayout.DefaultLayout, {
      tools: tools
    });
