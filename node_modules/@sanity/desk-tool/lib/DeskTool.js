...
function DeskTool(props) {
  ...
  var paneKeys = (0, _react.useMemo)(() => {
    return routerPanes.reduce((ids, group) => ids.concat(group.map(sibling => sibling.id)), ['root']);
  }, [routerPanes]);
  var paneGroups = (0, _react.useMemo)(() => [[{
    id: 'root',
    params: {}
  }]].concat(routerPanes || []), [routerPanes]);
  var groupIndexes = (0, _react.useMemo)(() => routerPanes.reduce((ids, group) => ids.concat(group.map((_, groupIndex) => groupIndex)), []), [routerPanes]);
  ...
  return /*#__PURE__*/_react.default.createElement(_deskTool.DeskToolProvider, {
    ...
    return (
      ...
      _react.default.createElement(_react.Fragment, {
        ...
        var pane = resolvedPanes[++i];
        if (!pane) return null;
        path = path.concat([pane.id || "[".concat(i, "]")]);
